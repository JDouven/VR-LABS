...

{
	...
	sum += tex2D(_GrabTexture, float2(screenPos.x + 5.0 * depth, screenPos.y + 5.0 * depth)) * 0.05;
	sum += tex2D(_GrabTexture, float2(screenPos.x - 5.0 * depth, screenPos.y - 5.0 * depth)) * 0.05;

	sum += tex2D(_GrabTexture, float2(screenPos.x + 4.0 * depth, screenPos.y + 4.0 * depth)) * 0.07;
	sum += tex2D(_GrabTexture, float2(screenPos.x - 4.0 * depth, screenPos.y - 4.0 * depth)) * 0.07;

	sum += tex2D(_GrabTexture, float2(screenPos.x + 3.0 * depth, screenPos.y + 3.0 * depth)) * 0.1;
	sum += tex2D(_GrabTexture, float2(screenPos.x - 3.0 * depth, screenPos.y - 3.0 * depth)) * 0.1;


	sum += tex2D(_GrabTexture, float2(screenPos.x + 2.0 * depth, screenPos.y + 2.0 * depth)) * 0.1;
	sum += tex2D(_GrabTexture, float2(screenPos.x - 2.0 * depth, screenPos.y - 2.0 * depth)) * 0.1;

	sum += tex2D(_GrabTexture, float2(screenPos.x + 1.0 * depth, screenPos.y + 1.0 * depth)) * 0.16;
	sum += tex2D(_GrabTexture, float2(screenPos.x - 1.0 * depth, screenPos.y - 1.0 * depth)) * 0.16;

	sum += tex2D(_GrabTexture, float2(screenPos.x + 0.5 * depth, screenPos.y + 0.5 * depth)) *  0.2;
	sum += tex2D(_GrabTexture, float2(screenPos.x - 0.5 * depth, screenPos.y - 0.5 * depth)) *  0.2;
